...
import com.example.commutedocmaker.ui.views.convertMinutesToStringTime
import com.example.commutedocmaker.ui.views.convertStringTimeToQuarters
import com.example.commutedocmaker.xlsx.XLSXConverter.CommuteSheetDataType
import com.example.commutedocmaker.xlsx.XLSXConverter.CommuteSheetDataType.*
...
class XLSXConverter(private val context: Context) {
                ...
                for (date in dates) {
                    if(forthRouteIncluded){
                        val row: RowData = mapOf(
                            ...
                            DISTANCE_TRAVELLED to distanceTravelled.toString(),
                            EURO_PER_KM_RATE to cashPerKilometer.toString().replace('.', ',', ignoreCase = true),
                            TOTAL_EURO to (distanceTravelled * cashPerKilometer).toString(),
    ...
    enum class CommuteSheetDataType(val id: Int, val columnWidth: Int, val tag: String = "") {
